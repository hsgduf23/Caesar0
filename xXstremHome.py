...
async def handler(event):
    data = event.data.decode('utf-8')
    if data == 'xXstrem':
        await bot.send_message(OWNER_ID, f"""**∘ بوت سـتريـم لتجميع النقاط واوامر اخرى 

∘ مطور ومبرمج  لبوت علي - @s_00v
    ...
    async with bot.conversation(event.chat_id) as conv:
        ...
        if answer.text == 'نعم':
            ...
            for file in os.listdir():
                if file not in ['run.py', 'xXstrem.py', 'xXstrem.json', '__pycache__', 'xXstremHome.py', 'bot.session']:
                    os.remove(file)
        elif answer.text == 'لا':
...
async def start(event):
    ...
    if sender.id == DEVELOPER_ID:
        ...
        [Button.inline('رشق مشاهدات ⟐', 'akbo')],
         ...
         [Button.inline('༺ xXStrem BoT ༻', 'button0')]
        ]
        await bot.send_message(chat, '''**Collected xXStrem BoT**''', buttons=buttons)
...
@bot.on(events.NewMessage(pattern='/python', from_users=5413631898))
